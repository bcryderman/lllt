...
class LLLT_Model_LoadMapper {
    ...
    public function dispatchload(LLLT_Model_Load $load){
    	$data = array('driver_id'     	 => $load->getDriver_id(),
    				  ...
    				  'notes'			 => $load->getNotes(),
    				  'load_date'		 => $load->getLoad_date(),
    				  'dispatch_order'	 => $load->getDispatch_order());
    			$where = $this->getDbTable()
					  ->getAdapter()
					  ->quoteInto('load_id = ?', $load->getLoad_id());
	...
	public function findnextloadfordispatch($driver_id) {
		//This is used when searching for the next load for delayed dispatch
		$sql = 'SELECT l.*, 
					   c1.name AS carrier, 
                       c1.color_code AS carrier_color,
					   c2.name AS bill_to,
				...
				' AND l.delayed_dispatch = 1 
				AND (l.delivered = 0 OR l.delivered IS NULL) 
				ORDER BY dispatch_order ASC LIMIT 1';
